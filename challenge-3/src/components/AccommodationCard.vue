<template>
  <div class="mx-auto px-4 w-2/4 lg:w-1/3">
        <div class="bg-white shadow-xl rounded-lg mb-6 tracking-wide" >
            <div class="md:flex-shrink-0">
                <img :src="accommodation.imgUrl" class="w-full h-48 rounded-lg rounded-b-none">
            </div>
            <div class="px-4 py-2 mt-2">
                <span class="flex flex-row">
                  <span class="inline-block rounded-full text-white bg-purple-500 py-1 px-4 py-1 font-bold mr-3 text-white text-sm mb-2">
                    {{ accommodation.Type.name }}
                  </span>
                  <font-awesome-icon class="ml-auto text-gray-500 cursor-pointer mt-1" :icon="['far', 'bookmark']" />
                </span>
                <h2 class="font-bold text-lg text-gray-800 tracking-normal mb-1">{{ accommodation.name }}</h2>
                ...
                <p class="text-xs text-gray-700 mr-1 mb-2 font-bold">
                    {{ getFormatedCurrency }}
                </p>
            </div>
        </div>
...
export default {
  ...
  computed: {
    getFormatedCurrency() {
      return `Rp. ${new Intl.NumberFormat('ID').format(this.accommodation.price)} / day`;
    },
    getFormatedFacility() {
